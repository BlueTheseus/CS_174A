...
Below is a screen recording demonstrating a similar demo for this assignment (If you can't visualze it, you can look at the video-1.mov file in the Assignment2 folder)
...
If you run the code as is, you should be able to see the front and left face of a cube. If you cannot see the left face, make sure you are looking at it from the left side by mouse dragging. This is due to backface culling, something we will learn about soon.
...
**Exercise 2 (5 points): Tilt each cubes 15 degrees to the left with respect to the previous cube in the stack. Make sure the top left corner of each cube is in perfect contact to the bottom left corner of the next cube.** Look at discussion slides for hints.
...
MAX_ANGLE = 15 * 2*Math.PI/360 // 15 degrees converted to radians
T = 3 // oscillation persiod in seconds
...
**Exercise 4 (2.5 points)** Make the cube swing to the left and back to the center. The swinging needs to be smooth, and in particular it has to followin a sinusoidal curve that goes from 0 to 15 degrees with a total period of oscillation of 3 seconds. The cubes must never swing right.
...
2. Exercise 2: Tilt the cubes 15 degrees to the left with respect to the previous cube in the stack. 5 points.
3. Exercise 3: Make the cubes taller using the scaling matrix transform. 5 points.
4. Exercise 4: Make the cube swing to the left and back to the center with a correct period of oscillation. 2.5 points.
